...
在非 layer-shell 桌面上这是**预期行为**。浮层照样能用，只是变成了飘浮透明窗口而不是边缘锚定层。要固定到你想要的位置，见 [[Window-Position.zh-CN#在-gnome--kde-上固定窗口降级模式]]。

在 layer-shell 合成器上你**故意**要降级时想消掉这个提示？那也是正常的——提示只在降级模式打印。

## 有气泡但位置 / 显示器不对

- layer-shell 合成器上，位置由 `style.align`、`general.margin`、`general.margin-horizontal` 决定。见 [[Window-Position.zh-CN]]。
...
- 记住查找顺序：`--config <path>` → `<cwd>/seekey.ini` → 默认值。没有 `~/.config/` 回退，除非你主动要。见 [[Configuration.zh-CN]]。

## Matugen 颜色没生效

- seekey 不监视 JSON。重跑 `matugen`，然后**重启** seekey。见 [[Matugen-Integration.zh-CN]]。
...
按你的发行版装依赖——见 [[Build-from-Source.zh-CN#1-安装依赖]]。或直接跑 `./install.sh`，它帮你装依赖。
